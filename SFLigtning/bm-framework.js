...
(function(context) {
    ...
    require(["domReady"], function(domReady) {
      ...
      bootstrap(context);
        ...
        var expose = function(varname, val, varname2) {
            // yield to require if it already exists
            if (varname2 && window[varname]) {
                window[varname][varname2] = window[varname][varname2] || val;
            } else {
                window[varname] = window[varname] || val;
            }
        };
        // We use the bmref namespace to avoid conflicts with developer builds. Production sites do not do this.
        expose("bmref", {});
        expose("bmref", require, "require");
        expose("bmref", define, "define");
        expose("bmref", requirejs, "requirejs");
    };
    /**
    ...
    bootstrap = function(context) {
        ...
        /**
         * Replacement for jquery.cookie
         */
         if (!Bm.Common.CookieUtils){
             Bm.Common.CookieUtils = {
                 checkCookie: function(name){
                     var i, dc = document.cookie.split(/\s*;\s*/);
                     dc = dc.map(function(cookie){
                         return cookie.split(/\s*=\s*/);
                     });
                     for(i = 0; i < dc.length; i++){
                         if (dc[i][0] === name) return dc[i][1];
                     }
                     return null;
                 },
                 setCookie: function(name, value, options){
                     if (typeof value !== undefined){
                         options = options || {};
                         var expires = options.expires ? '; expires=' + options.expires.toUTCString() : '',
                             path = options.path ? '; path=' + options.path : '',
                             domain = options.domain ? '; domain=' + options.domain : '',
                             secure = options.path ? '; secure=' + options.secure : '';
